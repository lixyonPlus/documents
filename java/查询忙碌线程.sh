...
printStackOfThreads() {
    ...
    while IFS=" " read -a line ; do
        ...
        local pcpu="${line[2]}"
        local cpu=`echo ${pcpu} | awk -F  "." '{print $1}'`
        local user="${line[3]}"
        ...
        if [[ $cpu -ge 5 ]];then
           {
            ...
            echo
           } | tee ${append_file:+-a "${append_file}_5"} ${store_dir:+-a "${store_file_prefix}$PROG"}
        elif [[ $cpu -ge 2 ]];then
           {
            echo "大于2小于5"
            bluePrint "[$idx] Busy(${pcpu}%) thread(${threadId}/${threadId0x}) stack of java process(${pid}) under user(${user}):"
            sed "$sed_script" -n ${jstackFile}
            echo
           } | tee ${append_file:+-a "${append_file}_2~5"} ${store_dir:+-a "${store_file_prefix}$PROG"}
        elif [[ $cpu -ge 1 ]];then
           {
            echo "大于1小于2"
            bluePrint "[$idx] Busy(${pcpu}%) thread(${threadId}/${threadId0x}) stack of java process(${pid}) under user(${user}):"
            sed "$sed_script" -n ${jstackFile}
            echo
           } | tee ${append_file:+-a "${append_file}_1~2"} ${store_dir:+-a "${store_file_prefix}$PROG"}
        else
           :
        fi
