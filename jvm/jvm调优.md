...
## 吞吐量优先的并行收集器,如上文所述，并行收集器主要以到达一定的吞吐量为目标，适用于科学技术和后台处理等。
...
## 响应时间优先的并发收集器,如上文所述，并发收集器主要是保证系统的响应时间，减少垃圾收集时的停顿时间。适用于应用服务器、电信领域等。
...
### 调优步骤
    查询资源占用最高的进程(得到进程id): top 
    查看占用资源最多的线程： top -Hp pid(进程id)
    jstack pid(进程id) 查看堆栈，重点关注：WAITING BLOCKED 
    ...
    jinfo 查看程序运行jvm参数
    jmap -histo pid | head -20 查询内存中前20行信息 【卡顿】
